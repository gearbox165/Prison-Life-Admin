...
Execute.PlaceholderText = "Press "..Prefix.." To Use Your Cmd"
Execute.Text = ""
...
Cmd[#Cmd + 1] =	{Text = "unloopkillguard / unloopkillsguard",Title = "Unloop kills all the guards"}
...
--[[if not Slient then
	Chat("Admin Commands By LocalPlayer   #   7434 modified by rareusername101lol")
...
function PlayerChatted(Message)
	...
	if Command("autogun") or Command("autoguns") or Command("autoallguns") then
		...
		Notify("You will automatically get guns upon respawn", Color3.fromRGB(0, 255, 0), "Success")
		if BuyGamepass then
			workspace.Remote.ItemHandler:InvokeServer(workspace.Prison_ITEMS.giver["Remington 870"].ITEMPICKUP)
			workspace.Remote.ItemHandler:InvokeServer(workspace.Prison_ITEMS.giver["M4A1"].ITEMPICKUP)
			workspace.Remote.ItemHandler:InvokeServer(workspace.Prison_ITEMS.giver["AK-47"].ITEMPICKUP)
	...
	if Command("chatnotify") then
		...
		Notify("TuIned on ChatNotify", Color3.fromRGB(0, 255, 0), "Success")
	end
	...
	if Command("killaura") then
		...
		Notify("Turned the kill aura on", Color3.fromRGB(0, 255, 0), "Success")
	end
	if Command("nokillaura") or Command("unkillaura") then
		States.Kill_Aura = false
		Notify("Turned the kill aura off", Color3.fromRGB(0, 255, 0), "Success")
	end
	...
	if Command("unadmin") or Command("removeadmin") then
		...
		if Player ~= nil and Admin[Player.UserId] then
			...
			Chat("/w "..Player.Name.." You Have Been Unranked! No Commands For You")
